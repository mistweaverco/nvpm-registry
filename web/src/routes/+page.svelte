<script lang="ts">
	...
	let availableEditorIntegrations: string[] = [];
	...
	let selectedEditorIntegrations: string[] = [];
	let languageInput = '';
	let categoryInput = '';
	let editorIntegrationInput = '';
	...
	$: availableEditorIntegrations = uniqSorted(packages.flatMap((p) => p.editor_integration ?? []));
	...
	const filterPackages = () => {
		...
		if (q !== '') {
			pkgs = pkgs.filter((pkg) => {
				...
				const editorMatch = (pkg.editor_integration ?? []).find((e) => e.toLowerCase().includes(q));
				if (editorMatch !== undefined) {
					pkg.searchMatchInfo = `Editor matched (${editorMatch})`;
					return true;
				}
				return false;
			});
		...
		if (selectedEditorIntegrations.length > 0) {
			const want = new Set(selectedEditorIntegrations.map((v) => v.toLowerCase()));
			pkgs = pkgs.filter((pkg) =>
				(pkg.editor_integration ?? []).some((e) => want.has(e.toLowerCase()))
			);
		}
	...
	const syncStateFromUrl = () => {
		...
		selectedEditorIntegrations = dedupeInsensitiveSorted(sp.getAll('editor').filter(Boolean));
		if (packages.length > 0) {
			...
			selectedEditorIntegrations = canonicalizeAgainstPool(
				selectedEditorIntegrations,
				availableEditorIntegrations
			);
		}
	...
	const syncUrlFromState = async (opts?: { includeQuery?: boolean }) => {
		...
		next.delete('editor');
		for (const e of selectedEditorIntegrations) next.append('editor', e);
	...
	const addEditorIntegration = (value: string) => {
		const v = value.trim();
		if (!v) return;
		const canon =
			availableEditorIntegrations.find((p) => p.toLowerCase() === v.toLowerCase()) ?? v;
		if (selectedEditorIntegrations.some((x) => x.toLowerCase() === canon.toLowerCase())) return;
		selectedEditorIntegrations = dedupeInsensitiveSorted([...selectedEditorIntegrations, canon]);
		editorIntegrationInput = '';
		filterPackages();
		void syncUrlFromState({ includeQuery: false });
	};

	const removeEditorIntegration = (value: string) => {
		const lower = value.toLowerCase();
		selectedEditorIntegrations = selectedEditorIntegrations.filter((x) => x.toLowerCase() !== lower);
		filterPackages();
		void syncUrlFromState({ includeQuery: false });
	};
	...
	onMount(async () => {
		...
		const editorPool = uniqSorted(sortedData.flatMap((p: Package) => p.editor_integration ?? []));
		...
		selectedEditorIntegrations = canonicalizeAgainstPool(selectedEditorIntegrations, editorPool);
		filterPackages();
	});
	...
	<div class="modal-box max-h-[90vh] max-w-3xl overflow-y-auto">
		...
		<table class="table w-full">
			<tbody>
				...
				{#if activePackageData.editor_integration?.length}
					<tr>
						<td>Editor integration:</td>
						<td>{activePackageData.editor_integration.join(', ')}</td>
					</tr>
				{/if}
	...
	<form on:submit|preventDefault={onSearchSubmit}>
		<div class="grid grid-cols-1 gap-4 md:grid-cols-4">
			...
			<TagFilter
				...
				selected={selectedEditorIntegrations}
				suggestions={availableEditorIntegrations}
				bind:inputValue={editorIntegrationInput}
				inputPlaceholder="Type an editor"
				showAllOptions={true}
				minCharsToShowOptions={0}
				onAdd={addEditorIntegration}
				onRemove={removeEditorIntegration}
			/>
		</div>
	</form>
</fieldset>
